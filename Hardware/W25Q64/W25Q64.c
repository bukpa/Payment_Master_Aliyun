...
{	
	...
	SPIX_SetSpeed(SPI_BaudRatePrescaler_2);//设置为72/2=36M时钟,高速模式
...
uint16_t W25QXX_ReadID(void)
{
	...
	SPIX_ReadWriteByte(0x00); 	//发送三个占位字节0x00，这些字节用于满足W25Q64的读取ID的操作要求，实际上不影响读取ID 
    
	SPIX_ReadWriteByte(0x00);
    
	SPIX_ReadWriteByte(0x00);
    
	Temp|=SPIX_ReadWriteByte(0xFF)<<8;   //ID的高8位
    
	Temp|=SPIX_ReadWriteByte(0xFF);	    //ID的低8位
