...
#if TLSF_USE_LOCKS
#include "main.h"
#define TLSF_CREATE_LOCK(_unused_)   do{}while(0)   //创建仅一次，不需要加锁
#define TLSF_DESTROY_LOCK(_unused_)  do{}while(0)   //没有销毁，不需要加锁
#define TLSF_ACQUIRE_LOCK(_unused_)  do{xSemaphoreTake(xTLSF_MallocLock_Mutex,0);}while(0)
#define TLSF_RELEASE_LOCK(_unused_)  do{xSemaphoreGive(xTLSF_MallocLock_Mutex);}while(0)
#else
